...
class Mascara {
	...
	public function __construct() {
		...
		foreach ($mascaras as $dados) {
			$this->criaMascara(
				...
				$dados->MASCARA,
				$dados->VALOR_PADRAO,
				($dados->IND_REVERSA = 0) ? false : true,
				$dados->FUNCAO,
				($dados->IND_MESMO_TAMANHO = 0) ? false : true,
				$dados->TIPO_MASCARA
			);
		}
	...
	private function criaMascara ($nome, $mascara, $valorPadrao = null, $reversa = false, $funcaoValidacao = false,$mesmoTamanho = true,$tipo = T_STRING_CAST) {
		...
		$this->_masks[$nome] = new \Zage\Adm\TipoMascara();
	...
	public function validaMascarasForm ($arqXml) {
		...
		$inputs = \Zage\Sys\DBApp::getXmlInputs($arqXml);
	...
	public function retiraMascarasForm ($arqXml) {
		...
		/** Seleciona todos os inputs do XML **/
		$inputs = \Zage\Sys\Util::getXmlInputs($arqXml);
    ...
    public static function validaCep ($string) {
    	$mascara	= "_____-___";
    	return (\Zage\Adm\Mascara::validaMascara($mascara, $string));
    }
    ...
    public function DBGetMascaras() {
    	...
    	return($db->extraiTodos("
			...
			FROM	ZGADM_MASCARA M,
					ZGADM_MASCARA_TIPO TM
			WHERE	M.COD_TIPO			= TM.CODIGO
			ORDER	BY M.NOME
			"));
    }
