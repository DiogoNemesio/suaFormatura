...
#################################################################################
if (!isset($codConvidado)) 		{
	\Zage\App\Erro::halt($tr->trans('Falta de Parâmetros').' (COD_CONVIDADO)');
}
...
try {
	...
	if (!$info) 	{
		\Zage\App\Erro::halt($tr->trans('Convidado não existe'));
	}else{
		$podeRemover	= null;
		$mensagem		= $tr->trans('Deseja realmente excluir o Convidado').': <em><b>'.$info->getNome().'</b></em> ?';
		$classe			= "text-warning";
	}
...
$tpl->set('TITULO'				,$tr->trans('Exclusão de Convidado'));
$tpl->set('ID'					,$id);
$tpl->set('TEXTO'				,$mensagem);
$tpl->set('MENSAGEM'			,$mensagem);
$tpl->set('CLASSE'				,$classe);
$tpl->set('VAR'					,'codConvidado');
