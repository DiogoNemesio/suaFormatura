...
#################################################################################
if (!isset($codEventoTipo)) 		{
	\Zage\App\Erro::halt($tr->trans('Falta de Parâmetros').' (COD_EVENTO_TIPO)');
}
...
try {
	...
	$infoR 	 = $em->getRepository('Entidades\ZgfmtEvento')->findOneBy(array('codTipoEvento' => $codEventoTipo));
	...
	if($infoR){
		...
		$mensagem		= $tr->trans('Tipo Evento "%s" está em uso e não pode ser excluído (TIPO)',array('%s' => $info->getDescricao()));
		$classe			= "text-danger";
	}else{
		$podeRemover	= null;
		$mensagem		= $tr->trans('Deseja realmente excluir o Tipo Evento').': <em><b>'.$info->getDescricao().'</b></em> ?';
		$classe			= "text-warning";
	}
...
$tpl->set('TITULO'				,$tr->trans('Exclusão de Tipo Evento'));
$tpl->set('ID'					,$id);
