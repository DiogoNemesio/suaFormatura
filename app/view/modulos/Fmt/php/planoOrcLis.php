...
$grid->adicionaTexto($tr->trans('DATA CADASTRO'),		20, $grid::CENTER	,'dataCadastro');
...
for ($i = 0; $i < sizeof($orcamento); $i++) {
	...
	if ($orcamento[$i]->getIndAtivo() == 1) {
		...
		$grid->setValorCelula($i, 1, "<span class=\"label label-danger arrowed-in\">Não Ativa</span>");
	}
	
	$grid->setUrlCelula($i,3,ROOT_URL.'/Fmt/planoOrcAlt.php?id='.$uid);
	$grid->setUrlCelula($i,4,ROOT_URL.'/Fmt/planoOrcExc.php?id='.$uid);
}
