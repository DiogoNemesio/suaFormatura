...
try {
	...
	if (!$info) 	{
		\Zage\App\Erro::halt($tr->trans('Rifa não existe'));
	}
	
	$podeRemover	= null;
	$mensagem		= $tr->trans('Deseja realmente excluir a rifa').': <b>'.$info->getNome().'</b> ?';
	$observacao		= $tr->trans('<i class="fa fa-exclamation-triangle red"></i> Está operação excluirá definitivamente a rifa do portal.');
	$classe			= "text-warning";
...
$urlVoltar			= ROOT_URL . "/Fmt/rifaLis.php?id=".$uid;
...
$tpl->set('TITULO'				,$tr->trans('Excluir Rifa'));
$tpl->set('ID'					,$id);
