...
for ($i = 0; $i < sizeof($rateios); $i++) {
	...
	$tabRateio	.= '<tr><td><select class="select2 '.$sView.'" style="width:100%;" name="codCategoria[]" data-rel="select2">'.$oCatInt.'</select></td><td><select class="select2 '.$sView.'" style="width:100%;" name="codCentroCusto[]" data-rel="select2">'.$oCentroCustoInt.'</select></td><td><input type="text" '.$tView.' '.$tView.' name="valorRateio[]" onchange="calculaPctCellRateioCPAlt($(this));" value="'.\Zage\App\Util::formataDinheiro(round($rateios[$i]->getValor(),2)).'" maxlength="20" autocomplete="off" zg-data-toggle="mask" zg-data-mask="dinheiro" zg-data-mask-retira="0"></td><td><input type="text" name="pctRateio[]" '.$tView.' '.$tView.' value="'.\Zage\App\Util::formataDinheiro(round($rateios[$i]->getPctValor(),2)).'" onchange="calculaValorCellRateioCPAlt($(this));" maxlength="7" autocomplete="off" zg-data-toggle="mask" zg-data-mask="porcentagem" zg-data-mask-retira="0"></td><td class="center"><span class="center '.$hView.'" '.$bView.' zgdelete onclick="delRowRateioCPAlt($(this));"><i class="fa fa-trash bigger-150 red"></i></span><input type="hidden" name="codRateio[]" value="'.$rateios[$i]->getCodigo().'"></td></tr>';
}
