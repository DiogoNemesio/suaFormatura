...
try {
	...
	->leftJoin('\Entidades\ZgfinCategoria'		,'cap', \Doctrine\ORM\Query\Expr\Join::WITH, 'ca.codCategoriaPai 	= cap.codigo')
...
for ($i = 0; $i < sizeof($result); $i++) {
	...
	$catPai	= $result[$i]["CATEGORIA_PAI_DESCRICAO"];
	
	
	if (empty($result[$i]["CATEGORIA_PAI_DESCRICAO"]) && empty($result[$i]["CATEGORIA_DESCRICAO"]) ) {
		$cat	= null;
	}elseif (empty($result[$i]["CATEGORIA_PAI_DESCRICAO"])) {
		$cat	= $result[$i]["CATEGORIA_DESCRICAO"];
	}else{
		$cat	= $result[$i]["CATEGORIA_PAI_DESCRICAO"].'/'.$result[$i]["CATEGORIA_DESCRICAO"];		
	}
	
	
	#################################################################################
	## Guardar a informação de parentesco da categoria
	#################################################################################
	$infoCat[$cat]		= $result[$i]["CATEGORIA_PAI_DESCRICAO"];
	...
	if (!isset($totalSubCat[$cat]))	$totalSubCat[$cat]		= 0;
	if (!isset($totalCat[$cat]))	$totalCat[$cat]			= 0;
		
	
	
	$dados[$cat][$n]["VENCIMENTO"]	= $venc;
	$dados[$cat][$n]["VALOR"]		= $valor;
	$dados[$cat][$n]["PARCELA"]		= $parc;
	$dados[$cat][$n]["DESCRICAO"]	= $result[$i]["DESCRICAO"];
	$dados[$cat][$n]["STATUS"]		= $result[$i]["STATUS_DESCRICAO"];
	$dados[$cat][$n]["PESSOA"]		= $result[$i]["PESSOA_NOME"];
	$dados[$cat][$n]["CAT_PAI"]		= $result[$i]["CATEGORIA_PAI_DESCRICAO"];

	
	#################################################################################
	## Atualizar os totalizadores
	#################################################################################
	$totalSubCat[$cat]		+= \Zage\App\Util::to_float($result[$i]["VALOR"]);
	...
	if (!empty($result[$i]["CATEGORIA_PAI_DESCRICAO"])) {
		$totalCat[$result[$i]["CATEGORIA_PAI_DESCRICAO"]]	+= \Zage\App\Util::to_float($result[$i]["VALOR"]);
	}
...
if (sizeof($dados) > 0) {
	...
	#################################################################################
	## Registrar a categoria pai para mostrat o resumo
	#################################################################################
	$categoriaPai	= null;
	
	foreach ($dados as $cat => $dado) {
		if (empty($cat))	{
			$categoria	= "(sem categoria)";
			$textoCat	= $categoria;
		}else{
			$categoria	= $cat;
			$textoCat	= 'da categoria "'.$cat.'"';
		}

		#################################################################################
		## Verificar se é para mostrar o resumo da categoria Pai
		#################################################################################
		if (($categoriaPai !== $infoCat[$cat]) && ($categoriaPai !== null)) {
			$tSubCat		= isset($totalSubCat[$categoriaPai])	? $totalSubCat[$categoriaPai] 	: 0;
			$tCat			= isset($totalCat[$categoriaPai]) 		? $totalCat[$categoriaPai]		: 0;
			$totalGeral		= \Zage\App\Util::to_money($tSubCat + $tCat); 
			$table .= '</tbody><thead><tr><th style="text-align: right;" colspan="5"><strong>Total geral da categoria "'.$categoriaPai.'": </strong></th><th style="text-align: right;" ><strong>'.$totalGeral.'</strong></th></tr></thead><tbody>';
		}
		
		
		$table .= '</tbody><thead><tr><th style="text-align: left;" colspan="5"><strong>'.$categoria.'</strong></th><th style="text-align: right;"><strong>'.\Zage\App\Util::to_money($totalSubCat[$cat]).'</strong></th></tr></thead><tbody>';
		...
		$table .= '<tr><th style="text-align: right;" colspan="5"><strong>Total '.$textoCat.' ('.$numCat.'):</strong></th><th style="text-align: right;" ><strong>'.\Zage\App\Util::to_money($totalSubCat[$cat]).'</strong></th></tr>';
	}
