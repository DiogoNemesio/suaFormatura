...
$grid->adicionaMoeda($tr->trans('SALDO A PAGAR'),		20, $grid::CENTER	,'');
...
if (sizeof($aNaoPode) > 0) {
	...
	if (sizeof($aNaoPode) > 1) {
		$mensagem = $tr->trans('Contas não podem ser pagas, status não permitido');
	}else{
		$mensagem = $tr->trans('Conta não pode ser paga, status não permitido');
	}
}else{
	
	if ($contas[0]->getCodStatus()->getCodigo() == "P") {
		$inicioMensagem		= "Deseja realmente efetivar o pagamento do saldo ";
	}else{
		$inicioMensagem		= "Deseja realmente efetivar o pagamento ";
	}
...
#################################################################################
try {
	$aContaDeb	= $em->getRepository('Entidades\ZgfinConta')->findBy(array('codFilial' => $system->getCodEmpresa()),array('nome' => 'ASC'));
...
#################################################################################
$tpl->set('ID'					,$id);
$tpl->set('TITULO'				,'Pagamento de Contas em Lote');
