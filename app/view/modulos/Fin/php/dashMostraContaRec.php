...
$aStatus	= array("A","P");
try {
	$contas	= \Zage\Fin\ContaReceber::busca($dataIni,$dataFim,"V",null,null,null,$aStatus,null,null,null,null,null); 
} catch (\Exception $e) {
...
for ($i = 0; $i < sizeof($contas); $i++) {
	...
	#################################################################################
	if ($status == "C") {
		$valTotal	= ( floatval($contas[$i]->getValor()) + floatval($contas[$i]->getValorJuros()) + floatval($contas[$i]->getValorMora()) - (floatval($contas[$i]->getValorDesconto())) );
	}else{
	...
	$urlCon			= "javascript:zgAbreModal('".ROOT_URL."/Fin/contaReceberRec.php?id=".$uid."');";
...
if (!empty($htmlDiv)) {
	$htmlDiv	.= '<div class="row small">
		...
		<div class="col-sm-3 green"><div class="pull-right"><strong>'.\Zage\App\Util::to_money($valorTotalMes).'</strong></div></div>
	</div>
	';
}
