...
if (sizeof($creditos) > 0 || sizeof($debitos) > 0) {
	...
	foreach ($debitos as $lanc) {
		...
		$valDeb			+= \Zage\App\Util::to_float($lanc["VALOR"]);
	...
	foreach ($creditos as $lanc) {
		...
		#################################################################################
		$valTotal		+= \Zage\App\Util::to_float($lanc["VALOR"]);
		$valCre			+= \Zage\App\Util::to_float($lanc["VALOR"]);
		
	}
	$table .= '</tbody><thead><tr><th style="text-align: center;" colspan="6"><h6>RESULTADO</h6></th></tr></thead>';
	$table .= '<tfoot><tr><th style="text-align: right;" colspan="5"><strong>Total dos créditos:</strong></th><th style="text-align: right;"><strong>'.\Zage\App\Util::to_money($valCre).'</strong></th>';
	$table .= '<tr><th style="text-align: right;" colspan="5"><strong>Total:</strong></th><th style="text-align: right;"><strong>'.\Zage\App\Util::to_money($valCre + $valDeb).'</strong></th>';
	$table	.= "</tfoot></table>";
