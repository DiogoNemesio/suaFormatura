...
try {
	...
	if (!$subgrupo) 	{
		\Zage\App\Erro::halt($tr->trans('Sub-Grupo não existe'));
	}
	
	$produtos		= \Zage\Est\Subgrupo::listaProdutos($codSubgrupo);
	
	if (!$produtos) {
		$podeRemover	= null;
		$mensagem		= $tr->trans('Deseja realmente excluir o SubGrupo').': <em><b>%NOME%</b></em> ?';
		$classe			= "text-warning";
	}else{
		$podeRemover	= 'disabled';
		$mensagem		= $tr->trans('SubGrupo %NOME% não pode ser excluído',array('%NOME%' => $subgrupo->getDescricao()));
		$classe			= "text-danger";
	}
...
$tpl->set('NOME'				,$subgrupo->getDescricao());
