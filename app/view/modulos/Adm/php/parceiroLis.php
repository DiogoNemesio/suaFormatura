...
$grid->adicionaTexto($tr->trans('NOME/FANTASIA'),	 	15, $grid::CENTER	,'fantasia');
...
$grid->adicionaIcone(null,'fa fa-user blue',$tr->trans('Cadastro de usuários'));
$grid->adicionaIcone(null,'fa fa-graduation-cap green',$tr->trans('Plano para as formaturas'));
$grid->adicionaIcone(null,'fa fa-money green',$tr->trans('Configurar comissão'));
...
for ($i = 0; $i < sizeof($organizacoes); $i++) {
	...
	$grid->setUrlCelula($i,7,ROOT_URL.'/Adm/parceiroComissaoLis.php?id='.$uid);
	$grid->setUrlCelula($i,8,ROOT_URL.'/Adm/parceiroAlt.php?id='.$uid);
	$grid->setUrlCelula($i,9,ROOT_URL.'/Adm/parceiroExc.php?id='.$uid);
}
